    ...
    try {
      if (accessableCount >= 0) {
        const res = await Api.post(
          `/post/${commentInfo.postId}/comments`,
          writeComment
        );
  ...
  const onClickDelete = async () => {
    try {
      ...
      const res = await Api.delete(
        `/post/${commentInfo.postId}/comments/ ${commentInfo.id}`,
        config
      );
    ...
    <>
      <div className={cx("Profile")}>
        <img
          ...
          src={commentInfo.imageUrl}
          onClick={() => navigate(`/mypage/${commentInfo.memberId}`)}
        ></img>
        <div className={styles.ProfileInfo}>{commentInfo.userID}</div>
      ...
      <div className={styles.comments}>{commentInfo.content}</div>
      <div className={styles.time}>{commentInfo.time}</div>
      {enableReComment ? (
...
const Comments = (commentInfo: CommentProps) => {
  //받는 props가 CommentProps임을 알려준다.
      ...
      {commentInfo.className === "primary" && (
        <div className={cx(commentInfo.className)}>
          <PrimaryComment
            ...
            imageUrl={commentInfo.imageUrl}
            isAuthor={commentInfo.isAuthor}
            memberId={commentInfo.memberId}
          />
        </div>
      )}
