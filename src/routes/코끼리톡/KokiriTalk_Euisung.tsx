...
const KokiriTalk2 = () => {
  ...
  useEffect(() => {
    ...
    if (initialRender) {
      if (roomList && roomList.length > 0) {
        ...
        if (state != null) {
          //게시글에서 "코끼리톡으로 교환하기" 버튼을 클릭해서 들어온 경우
          ...
          //일반 코끼리톡 버튼을 클릭해서 들어온 경우 : 맨 위에 있는 쪽지방 선택
          getInitialTalkCardInfo();
  ...
  const createMessageRoom = async () => {
    try {
      ...
      const res = await Api.post(`/post/${postId}/messageRooms`, {
        postId,
        buyerId,
      });
      return res.data;
        ...
        <div className={styles.left}>
          ...
          <div className={styles.left2}>
            <div className={styles.talkContainer}>
              {isClicked === -1 && initialRoom ? (
                <div className={styles.wrapperOn}>
                  <TalkList2
                    partner={state.post.userInfoWithAddress.userDetail.nickname}
                    lastContent={""}
                    date={"방금전"}
              ...
              {roomList.map(
                (room, idx) =>
                  //클릭시 border 주기
                  (isClicked === idx && (
                    <div className={styles.wrapperOn} key={room.messageRoomId}>
                      <TalkList2
                        keys={room["messageRoomId"]}
          ...
          <div className={styles.right_headerBox}>
            <div className={styles.right_header}>
              <div className={styles.right_header1}>
                <div className={styles.right_header1_1}>
                  <TalkCard2 talkCardInfo={talkCardInfo} />
                </div>
            ...
            <div className={styles.right_header1_2}>
              <span
                className={styles.partnerName}
                onClick={() => navigate(`/mypage/${talkCardInfo.partnerId}`)}
              >
                {talkCardInfo.partnerName}
              </span>
              님과의 쪽지방입니다.
            </div>
          </div>
