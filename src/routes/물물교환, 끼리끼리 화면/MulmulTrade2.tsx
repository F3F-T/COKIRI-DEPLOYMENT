  ...
  return (
    <div className={styles.mulmulTrade}>
      <div className={styles.mulmulTradeContent}>
        {store.categoryReducer.category === "전체" ? (
          <div className={styles.navPostOrWant}>전체 게시글이에요.</div>
        ) : categories2.includes(store.categoryReducer.category) ? (
          <div className={styles.navPostOrWant}>
            {store.categoryReducer.category}을 원하는 사람들이에요 🙌
          </div>
        ) : (
          <div className={styles.navPostOrWant}>
            {store.categoryReducer.category}를 원하는 사람들이에요 🙌.
          </div>
        )}
        <div className={styles.popularOrNewest}>
          {tab1 === "next" ? (
            <button
              className={`${
                styles["newsetBtn" + (tab1 === "next" ? "active" : "")]
              }`}
              onClick={() => {
                setDealTab("next");
          ...
          {tab1 === "curr" ? (
            <button
              className={`${
                styles["pupularBtn" + (tab1 === "curr" ? "active" : "")]
              }`}
              onClick={() => {
                setDealTab("curr");
