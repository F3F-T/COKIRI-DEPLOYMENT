...
const SignUp = () => {
  ...
  async function CheckEmailDuplicated(email: object) {
    try {
      const res = await axios.post(
        "https://f3f-cokiri.site/auth/check-email",
        email
      );
      console.log(res);
      ...
      if (duplicated) {
        ...
        setValidationCheck((prevState) => {
          return {
            ...prevState,
            emailCheck: "duplicated",
            emailCheckBoolean: false,
          };
        });
      } //중복이 아닌 경우 -> false 반환
      else {
  ...
  async function CheckNickNameDuplicated(nickname: object) {
    try {
      const res = await axios.post(
        "https://f3f-cokiri.site/auth/check-nickname",
        nickname
      );
      const result = res.data;
      const duplicated = result.exists;
  ...
  async function CheckPhoneNumberDuplicated(phoneNumber: object) {
    try {
      const res = await axios.post(
        "https://f3f-cokiri.site/auth/check-phone",
        phoneNumber
      );
      const result = res.data;
      const duplicated = result.exists;
      ...
      if (duplicated) {
        ...
        setValidationCheck((prevState) => {
          return {
            ...prevState,
            phoneNumberCheck: "duplicated",
            phoneNumberCheckBoolean: false,
          };
        });
      } //중복이 아닌 경우 -> false 반환
      else {
        setValidationCheck((prevState) => {
          return {
            ...prevState,
            phoneNumberCheck: "valid",
            phoneNumberCheckBoolean: true,
          };
        });
        setuserInfo((prevState) => {
          return {
  ...
  const onChangeName = (e) => {
    ...
    if (inputName.length > 0) {
      ...
      setValidationCheck((prevState) => {
        return {
          ...prevState,
          nameAndBirthCheck: true,
          nameAndBirthCheckBoolean: true,
        };
      });
    }
    //한글자도 작성하지 않았을때 에러
      ...
      setValidationCheck((prevState) => {
        return {
          ...prevState,
          nameAndBirthCheck: false,
          nameAndBirthCheckBoolean: false,
        };
      });
    }
  };
  ...
  async function MailConfirm(jsonEmail: object) {
    try {
      const res = await axios.post(
        "https://f3f-cokiri.site/auth/mailConfirm",
        jsonEmail
      );
    ...
    <div className={styles.signup}>
      ...
      <div className={styles.userInfo}>
        ...
        {(validationCheck.emailCheck === undefined && (
          <Message
            validCheck={validationCheck.emailCheckBoolean}
            content={""}
          />
        )) ||
        ...
        {(validationCheck.passwordCheck === undefined && (
          <Message
            validCheck={validationCheck.passwordCheckBoolean}
            content={""}
          />
        )) ||
        ...
        <TextInput
          type={"password"}
          placeholder={"비밀번호 확인"}
          onBlur={onChangeCheckPassword}
        />
        {(passwordReCheck === undefined && (
          <Message validCheck={passwordReCheck} content={""} />
        )) ||
          (passwordReCheck ? (
            <Message
              validCheck={passwordReCheck}
              content={"✔ 비밀번호가 일치합니다"}
            />
          ) : (
            <Message
        ...
        {(validationCheck.nameAndBirthCheck === undefined && (
          <Message
            validCheck={validationCheck.nameAndBirthCheckBoolean}
            content={""}
          />
        )) ||
        ...
        {(validationCheck.nicknameCheck === undefined && (
          <Message
            validCheck={validationCheck.nicknameCheckBoolean}
            content={""}
          />
        )) ||
        ...
        {(validationCheck.phoneNumberCheck === undefined && (
          <Message
            validCheck={validationCheck.phoneNumberCheckBoolean}
            content={""}
          />
        )) ||
